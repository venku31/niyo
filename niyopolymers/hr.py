...
def send_mail_to_employees_on_shift():
    ...
    print(to_time)
    shift = frappe.db.sql("""select name from `tabShift Type` where HOUR(start_time) = %d """,(int(now_datetime.hour) - 1))
    if shift:
        ...
        checkin = frappe.db.sql("""select name,employee_name,time from `tabEmployee Checkin` where date(time) = %s and shift = %s """ ,(frappe.utils.nowdate(),doc.name),as_dict=1)
