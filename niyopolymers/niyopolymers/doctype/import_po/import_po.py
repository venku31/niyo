...
class ImportPO(BuyingController):
	def __init__(self, *args, **kwargs):
		super(ImportPO, self).__init__(*args, **kwargs)
		self.status_updater = [{
			'source_dt': 'Import PO Item',
	...
	def validate_with_previous_doc(self):
		super(ImportPO, self).validate_with_previous_doc({
			"Supplier Quotation": {
	...
	def check_modified_date(self):
		mod_db = frappe.db.sql("select modified from `tabImport PO` where name = %s",
			self.name)
...
def close_or_unclose_purchase_orders(names, status):
	if not frappe.has_permission("Import PO", "write"):
		frappe.throw(_("Not permitted"), frappe.PermissionError)

	names = json.loads(names)
	for name in names:
		po = frappe.get_doc("Import PO", name)
		if po.docstatus == 1:
			if status == "Closed":
...
def make_purchase_receipt(source_name, target_doc=None):
	...
	doc = get_mapped_doc("Import PO", source_name,	{
		"Import PO": {
...
def get_mapped_purchase_invoice(source_name, target_doc=None, ignore_permissions=False):
	...
	doc = get_mapped_doc("Import PO", source_name,	fields,
		target_doc, postprocess, ignore_permissions=ignore_permissions)

	return doc

@frappe.whitelist()
def make_rm_stock_entry(import_po, rm_items):
	...
	if import_po:
		import_po = frappe.get_doc("Import PO", import_po)
	...
	if fg_items:
		...
		stock_entry.import_po = import_po.name
		stock_entry.supplier = import_po.supplier
		stock_entry.supplier_name = import_po.supplier_name
		stock_entry.supplier_address = import_po.supplier_address
		stock_entry.address_display = import_po.address_display
		stock_entry.company = import_po.company
		stock_entry.to_warehouse = import_po.supplier_warehouse
...
def make_inter_company_sales_order(source_name, target_doc=None):
	...
	return make_inter_company_transaction("Import PO", source_name, target_doc)
...
def set_missing_values(source, target_doc):
	if target_doc.doctype == "Import PO" and getdate(target_doc.schedule_date) <  getdate(nowdate()):
		target_doc.schedule_date = None
